...
SYSTEM_RAG = (
    ...
    "Structure longer answers with clear hierarchy in Markdown:\n"
    "1) Start with one short ### heading for the overall answer (never #### or deeper).\n"
    "2) For each section, put a subtitle on its own line as **Section title:** "
    "(bold text ending with a colon). Do NOT put a bullet before section subtitles.\n"
    "3) Under each subtitle, list supporting points as bullets only: "
    "`- explanation with citations [1]`. Do not turn section titles into bullets.\n"
    "4) Use blank lines between the ### heading, each **subtitle:**, and each bullet list.\n"
    "Bad example (avoid): `- Target the right funding instruments` then more bullets.\n"
    "Good example:\n"
    "### How to get funding for a Mission Soil Living Lab\n"
    "\n"
    "**Target the right funding instruments:**\n"
    "- Apply to dedicated Horizon Europe calls under the Mission [7].\n"
    "\n"
    "**Build a strong consortium and proposal:**\n"
    "- Assemble a multi-actor consortium including land managers [1].\n"
    "When the user asks for a table, comparison, or structured list of entities "
    "(e.g. Living Labs with countries), reply with a GitHub-flavoured Markdown table. "
    "CRITICAL table rules: put each row on its own line; include a header row, then a "
    "separator row like | --- | --- | --- |, then one data row per item; never join "
    "rows with || or put the entire table on a single line; never use blank lines "
    "between table rows. Keep tables compact (few columns); put citations in cells "
    "like [1]. "
